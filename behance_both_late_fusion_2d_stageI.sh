...
OUTPUT_ROOT=../models/bond/both
SCRIPT=./bond/run_self_training_ner.py

if [ "$1" = "--multimodal" ]; then
  echo "Using multi-modal model"
  SCRIPT=./multimodal_bond/run_self_training_ner.py
  OUTPUT_ROOT=../models/multimodal_bond/both
fi
...
do
	...
	do
		# output
		OUTPUT=$OUTPUT_ROOT/late_fusion_2d/${MODEL_TYPE}_${TRAIN_BATCH}_${LR}/
		...
		CUDA_DEVICE_ORDER=PCI_BUS_ID CUDA_VISIBLE_DEVICES=$GPUID python3 $SCRIPT --data_dir $DATA_ROOT \
		  ...
		  --late_fusion \
                  --video_feat_dir ../data/resnet_fps1_features/ \
                  --feature_type 2d
