...
TOOL_LABEL_MAP = {0: 0, 1: 1, 3: 2, 2: 1, 4: 2}
CREATIVE_LABEL_MAP = {0: 0, 2: 1, 4: 2, 1: 0, 3: 0}
...
FEAT_DIR = '/home/code-base/scratch/data/resnet_fps1_features'
def read_examples_from_file(data_dir, mode):
    ...
    with open(file_path, 'r') as f:
        ...
        for item in data:
            ...
            assert len(words) == len(labels), (words, labels)
            ...
            #target_name_1 = os.path.join(FEAT_DIR, item["video_id"].replace("/", "__").replace(".mp4", ".%s.cut.mp4" % int(timestamp)))[:-4] + '.npy'
            #target_name_2 = os.path.join(FEAT_DIR, item["video_id"].replace("/", "__") + "%s.cut.mp4" % int(timestamp))[:-4] + '.npy'
            #target_name_3 = os.path.join(FEAT_DIR, item["video_id"].replace("/", "__").replace(".mp4", ".%s.cut.%s.mp4" % (int(timestamp), 10)))[:-4] + '.npy'
            #target_name_4 = os.path.join(FEAT_DIR, item["video_id"].replace("/", "__").replace(".mp4",".%s.cut.%s.mp4" % (int(timestamp), 20)))[:-4] + '.npy'
            #if os.path.exists(target_name_4):
            #    video_feat_file = target_name_4
...
#                 video_feat_file = target_name_3
            ...
            #    continue
            examples.append(
                InputExample(guid="%s-%d".format(mode, guid_index), words=words, labels=labels, hp_labels=hp_labels))
            guid_index += 1
...
def convert_examples_to_features(
    ...
    logger.info("Found %d extra long examples of %d", extra_long_samples, len(examples))
    return features
