...
TOOL_LABEL_MAP = {0: 0, 1: 1, 3: 2, 2: 0, 4: 0}
CREATIVE_LABEL_MAP = {0: 0, 2: 1, 4: 1, 1: 0, 3: 0}

class VideoNERDataset(Dataset):
    def __init__(self, text_feature_dataset, video_feature_files, video_feature_max_len, feature_type):
        self.text_feature_dataset = text_feature_dataset
        self.video_feature_files = video_feature_files
        self.video_feature_max_len = video_feature_max_len
    ...
    def _prepare_video_features(self, features):
        if features.shape[0] >= self.video_feature_max_len:
            features = features[:self.video_feature_max_len, :]
            mask = [1] * self.video_feature_max_len
        else:
            pad_length = self.video_feature_max_len - features.shape[0]
    ...
    def __getitem__(self, idx):
        ...
        if self.feature_type == '3d':
            ...
            video_feature, video_mask = self._prepare_video_features(np.load(self.video_feature_files[idx])['features'])
...
class InputExample(object):
    ...
    def __init__(self, guid, words, labels, hp_labels, video_feature_file):
...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, label_ids, full_label_ids, hp_label_ids, video_feature_file):
...
def read_examples_from_file(args, mode):
    ...
    if args.feature_type in ['slowfast', '2d']:
        suffix = '.npz'
    else:
        suffix = '.npy'
    ...
    with open(file_path, 'r') as f:
        ...
        for item in data:
            ...
            word_label_pairs = [(w, l) for w, l in zip(words, labels) if not w.isspace()]
            words = [p[0] for p in word_label_pairs]
            labels = [p[1] for p in word_label_pairs]

            if 'tool' in file_path:
                if any([l in [2,4] for l in labels]):
#                     print(item)
                    pass
                labels = [TOOL_LABEL_MAP[l] for l in labels]
            elif 'creative' in file_path:
                labels = [CREATIVE_LABEL_MAP[l] for l in labels]
            else:
                pass
...
def load_and_cache_examples(args, tokenizer, labels, pad_token_label_id, mode):
    ...
    cached_features_file = os.path.join(
        ...
        "cached_{}_{}_{}_{}_{}".format(
            args.feature_type, args.clip_length, mode, list(filter(None, args.model_name_or_path.split("/"))).pop(), str(args.max_seq_length)
        ),
    )
    ...
    text_feature_dataset = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids,
                                         all_full_label_ids,
                                         all_hp_label_ids, all_ids)
    video_ner_dataset = VideoNERDataset(text_feature_dataset, video_feature_files, args.video_feature_max_len, args.feature_type)

    return video_ner_dataset


def get_labels(path=None):
    if path and os.path.exists(os.path.join(path, "tag_to_id.json")):
        labels = []
        with open(os.path.join(path, "tag_to_id.json"), "r") as f:
            data = json.load(f)
        ...
        return ["O", "B-TOOL", "B-CREATIVE", "I-TOOL", "I-CREATIVE"]
...
def tag_to_id(path=None):
    if path and os.path.exists(os.path.join(path, "tag_to_id.json")):
        ...
        return {"O": 0, "B-TOOL": 1, "B-CREATIVE": 2, "I-TOOL": 3, "I-CREATIVE": 4}
