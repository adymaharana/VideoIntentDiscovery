...
from clf_data_utils import load_and_cache_examples
...
MODEL_CLASSES = {
    ...
    "roberta": (RobertaConfig, RobertaForSequenceClassification_v2, RobertaTokenizer),
...
def train(args, train_dataset, model_class, config, tokenizer):
    """ Train the model """
    ...
    for epoch in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            inputs = {"input_ids": batch[0], "attention_mask": batch[1], "labels": batch[2].float(),
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.evaluate_during_training:
                        ...
                        results, _, best_dev, _ = evaluate(args, model, tokenizer, best_dev,
                                                           mode="dev",
                        ...
                        results, _, best_test, is_updated = evaluate(args, model, tokenizer, best_test,
                                                                     mode="test",
...
def evaluate(args, model, tokenizer, best, mode, prefix="", verbose=True):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        nb_eval_steps += 1
        if preds is None:
            preds = torch.round(torch.sigmoid(logits)).detach().cpu().numpy()
            out_label_ids = inputs["labels"].detach().cpu().numpy()
        else:
            preds = np.append(preds, torch.round(torch.sigmoid(logits)).detach().cpu().numpy(), axis=0)
            out_label_ids = np.append(out_label_ids, inputs["labels"].detach().cpu().numpy(), axis=0)

    eval_loss = eval_loss / nb_eval_steps
    accuracy = simple_accuracy(preds, out_label_ids)
    ...
    logger.info("  Accuracy = %s", accuracy*100)

    return accuracy, preds, best, is_updated
...
def main():
    ...
    # multi-modal
    parser.add_argument('--video_embed_dim', type=int, default=2048,
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model.to(args.device)
            result, _, best_dev, _ = evaluate(args, model, tokenizer, pad_token_label_id, best=best_dev,
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        ...
        if not best_test:
            best_test = 0
        result, predictions, _, _ = evaluate(args, model, tokenizer, pad_token_label_id, best=best_test,
                                             mode="test")
        # Save results
