...
from model.bond.data_utils import load_and_cache_examples, tag_to_id, get_chunks
...
def evaluate(args, model, tokenizer, labels, pad_token_label_id, best, mode, prefix="", verbose=True):
    ...
    max_preds = np.argmax(preds, axis=2)

    label_map = {i: label for i, label in enumerate(labels)}
    preds_list = [[] for _ in range(out_label_ids.shape[0])]
    out_id_list = [[] for _ in range(out_label_ids.shape[0])]
    preds_id_list = [[] for _ in range(out_label_ids.shape[0])]
    pred_values = [[] for _ in range(out_label_ids.shape[0])]

    for i in range(out_label_ids.shape[0]):
        for j in range(out_label_ids.shape[1]):
            if out_label_ids[i, j] != pad_token_label_id:
                preds_list[i].append(label_map[max_preds[i][j]])
                out_id_list[i].append(out_label_ids[i][j])
                preds_id_list[i].append(max_preds[i][j])
                pred_values[i].append(preds[i][j])
    ...
    return results, (preds_list, pred_values), best, is_updated