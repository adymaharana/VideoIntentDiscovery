...
#                     target_name_3 = os.path.join(args.video_feat_dir, item["video_id"].replace("/", "__").replace(".mp4", ".%s.cut.%s.mp4" % (int(timestamp), 10)))[:-4] + '.npy'
                    #target_name_4 = os.path.join('../data/resnet_fps1_features/', item["video_id"].replace("/", "__").replace(".mp4", ".%s.cut.%s.mp4" % (int(timestamp), 20)))[:-4] + '.npy'
                    # if os.path.exists(target_name_1) or os.path.exists(target_name_2) or os.path.exists(target_name_3):
                    #     pass
                    #if os.path.exists(target_name_4):
                    #    pass
                    #else:
                    #    continue
                    prediction_dict[example_id] = {"str_words": item["str_words"],
                                               "predictions": predictions[0][example_id],
						"confidence": str(np.mean(predictions[1][example_id]))}
                    example_id += 1
