...
from model.multimodal_bond.eval import evaluate
...
def main():
    ...
    parser.add_argument('--feature_type', type=str, default='3d', help='Choose between 2d or 3d or slowfast')
    parser.add_argument('--feature_dir', type=str, default='3d', help='path to feature directory')
    parser.add_argument('--clip_length', type=str, default='3d', help='length of clip; used in getting feature file name')
    parser.add_argument('--video_feature_max_len', type=int, default=60, help='Max length of video features (depends on clip len or fps)')
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        with open('results.jsonl', 'a+') as fresult:
            for key in sorted(results.keys()):
                res_key = args.output_dir + '_' + key
                fresult.write(json.dumps({res_key: str(results[key])}) + '\n')
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        ...
        if args.feature_type in ['2d', 'slowfast']:
            suffix = '.npz'
        else:
            suffix = '.npy'
            ...
            with open(os.path.join(args.data_dir, "test.json"), "r") as f:
                ...
                for item in data:
                    ...
                    if os.path.exists(target_name):
                        ...
                        continue
                assert len(prediction_dict) == len(predictions)
