...
from modeling_roberta import RobertaForTokenClassification_v2, RobertaForTokenClassificationLateFusion
...
def initialize(args, model_class, config, t_total, epoch):
    ...
    if args.scheduler == 'linear':
        ...
        scheduler = ReduceLROnPlateau(optimizer, factor=0.5, patience=1, cooldown=1, min_lr=1e-6)
    else:
        raise ValueError
    ...
    for epoch in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if global_step >= args.self_training_begin_step:
                ...
                inputs = {"input_ids": batch[0], "attention_mask": batch[1], "video_embedding": batch[-2], "video_mask": batch[-1]}
                with torch.no_grad():
                ...
                inputs = {"input_ids": batch[0], "attention_mask": batch[1], "labels": pred_labels,
                          "label_mask": label_mask, "video_embedding": batch[-2], "video_mask": batch[-1]}
            else:
                inputs = {"input_ids": batch[0], "attention_mask": batch[1], "labels": batch[3],
                          "video_embedding": batch[-2], "video_mask": batch[-1]}
            ...
            if args.vat:
                ...
                if not word_embed:
                    ...
                    if (torch.isnan(norm) or torch.isinf(norm)):
                        ...
                        vat_logits, vat_final_embeds = vat_outputs[1], vat_outputs[2]
                        if args.vat_loss_type == "embeds":
                            vat_loss = get_mt_loss(vat_final_embeds, final_embeds.detach(), args.mt_class,
                                                   args.vat_lambda) \
                                       + get_mt_loss(final_embeds, vat_final_embeds.detach(), args.mt_class,
                                                     args.vat_lambda)
                        else:
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.evaluate_during_training:
                        ...
                        results, _, best_dev, _ = evaluate(args, model, tokenizer, labels, pad_token_label_id, best_dev,
                                                           mode="dev",
                        ...
                        results, _, best_test, is_updated = evaluate(args, model, tokenizer, labels, pad_token_label_id,
                                                                     best_test, mode="test",
...
def main():
    ...
    parser.add_argument("--scheduler", default='linear', type=str, help="Choose between ['linear', 'plateau'].")
    ...
    parser.add_argument('--self_training_begin_step', type=int, default=900,
    ...
    # multi-modal
    parser.add_argument('--video_embed_dim', type=int, default=2048, help='dimension of video embeddings')
    parser.add_argument('--video_feat_dir', type=str, required=True, help='path to directory for video features')
    parser.add_argument('--late_fusion', action="store_true", help='True if using the late fusion model')
    parser.add_argument('--feature_type', type=str, default='3d', help='Choose between 2d or 3d')
    ...
    if args.late_fusion:
        model_class = RobertaForTokenClassificationLateFusion
    ...
    config.ctx_dim = args.video_embed_dim
    ...
    # Training
    best_dev, best_test = 0, 0
    if args.do_train:
        train_dataset = load_and_cache_examples(args, tokenizer, labels, pad_token_label_id, mode="train")
        model, global_step, tr_loss, best_dev, best_test = train(args, train_dataset, model_class, config, tokenizer,
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model.to(args.device)
            result, _, best_dev, _ = evaluate(args, model, tokenizer, labels, pad_token_label_id, best=best_dev,
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        ...
        if not best_test:
            best_test = [0, 0, 0]
        result, predictions, _, _ = evaluate(args, model, tokenizer, labels, pad_token_label_id, best=best_test,
                                             mode="test")
        # Save results
        ...
        with open(output_test_predictions_file, "w") as writer:
            with open(os.path.join(args.data_dir, "test.json"), "r") as f:
                ...
                for item in data:
                    ...
                    #if mode != "test":
                    target_name_1 = os.path.join(args.video_feat_dir, item["video_id"].replace("/", "__").replace(".mp4", ".%s.cut.mp4" % int(timestamp)))[:-4] + '.npy'
                    target_name_2 = os.path.join(args.video_feat_dir, item["video_id"].replace("/", "__") + "%s.cut.mp4" % int(timestamp))[:-4] + '.npy'
                    target_name_3 = os.path.join(args.video_feat_dir, item["video_id"].replace("/", "__").replace(".mp4", ".%s.cut.%s.mp4" % (int(timestamp), 10)))[:-4] + '.npy'
                    if os.path.exists(target_name_1) or os.path.exists(target_name_2) or os.path.exists(target_name_3):
                        pass
                    else:
                        continue
                    prediction_dict[example_id] = {"str_words": item["str_words"],
                                                   ...
                                                   "predictions": predictions[example_id]}
                    example_id += 1
                json.dump(prediction_dict, writer)
