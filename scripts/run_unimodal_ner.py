...
from model.bond.modeling_roberta import RobertaForTokenClassification_v2
from model.bond.data_utils import load_and_cache_examples, get_labels
from model.bond.model_utils import multi_source_label_refine, soft_frequency, mt_update, get_mt_loss, opt_grad
from model.bond.eval import evaluate
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        ...
        with open(output_test_predictions_file, "w") as writer:
            with open(os.path.join(args.data_dir, "test.json"), "r") as f:
                example_id = 0
                data = json.load(f)
                print("Found %s samples in dataset and %s predictions" % (len(data), len(predictions[0])))
                for i, item in enumerate(data):
                    timestamp = float(item["timestamp"])
                    target_name = os.path.join('../data/behance_video_features/resnext101_fps=6', item["video_id"].replace("/", "__")[:-4] + ".%s.cut.%s" % (int(timestamp), 10) + '.npy')
                ...
                print("Saved %s predictions from %s outputs" % (len(prediction_dict), len(predictions[0])))
